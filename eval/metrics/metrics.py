...
class ObjectContactMetrics:
        ...
        if start_id == -1 or start_id == end_id:
            ...
            for frame in range(start_id, end_id):
                ...
                if penetr_mask.sum() == 0:
                    ...
                    contact_ratio = self.compute_contact_ratio_filter(obj_faces=obj_faces, obj_vertices=obj_vertices[frame],
                                                            sbj_vertices=sbj_vertices[frame], penetr_mask=penetr_mask.detach().cpu().numpy())
    ...
    def compute_contact_ratio_filter(self, obj_faces, obj_vertices, sbj_vertices, penetr_mask,in_contact_threshold=0.005):
        """
        nr. sbj vertices / nr. sbj vertices close to object (below in_contact_threshold)
        """
        n_sbj_vertices = sbj_vertices.shape[0]

        n_verts_in_contact = np.sum((self._dist_to_closets_point_on_obj < in_contact_threshold) * (penetr_mask != 1))
