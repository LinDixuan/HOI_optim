...
def smooth_mask(hand_pose_rec_o, hand_verts_o, mask):
    ...
    loss1 = 0.25 * torch.sum(
        (((hand_pose_rec[1:-1] - hand_pose_rec[:-2]) - (hand_pose_rec[2:] - hand_pose_rec[1:-1])) ** 2)) + \
...
def smooth_mask_pose(hand_pose_rec_o, mask):
    hand_pose_rec = torch.zeros_like(hand_pose_rec_o).to(hand_pose_rec_o.device)
    hand_pose_rec = hand_pose_rec + hand_pose_rec_o * mask
    hand_pose_rec = hand_pose_rec + hand_pose_rec_o.detach() * (1 - mask)

    loss1 = torch.sum(((hand_pose_rec[1:] - hand_pose_rec[:-1]) ** 2))
...
palm_center_right = [149, 152, 132,  77,  93, 147,  62,  69, 142, 148,  63,  66,  61,  60, 608,  67, 776,  94]
palm_center_left = [263, 150,  74, 149, 152, 141, 147, 197, 148,  63,  66,  65, 146]
...
def optimize1(name, fname='', res_path='.'):
    OBJ_MOTION_PATH = 'omomo/sequences_canonical'
    object_npz_path = os.path.join(OBJ_MOTION_PATH, fname, "object.npz")
    ...
    SMPLX_PATH = './models/smplx'
    export_file = os.path.join(res_path, fname)

    sbj_m = sbj_m_all[gender]

    OBJ_PATH = 'omomo/objects'
    ...
    sbj_face = sbj_m.f.detach().cpu().numpy()
    ...
    obj_normals = vertex_normals(verts_obj, torch.tensor(obj_info['faces'].astype(np.float32)).unsqueeze(0).repeat(
        verts_obj.shape[0], 1, 1).to(device))
    ...
    def calc_loss(verts, jtr, hand_pose_rec, epoch, left_or_right, body_n_pose):
        ...
        with torch.enable_grad():
            ...
            o2h_signed, sbj2obj, o2h_idx, sbj2obj_idx, o2h, sbj2obj_vector = point2point_signed(verts[:, hand_idx],
                                                                                                verts_obj,
                                                                                                y_normals=obj_normals,
                                                                                                return_vector=True)  # y_normals=obj_normals
            ...
            for i in range(16):
                sd_i = sbj2obj[:, HAND_INDEXES[i][HAND_INDEXES[i] < 778]]
                MASK_I = sd_i < thresh
                ...
                if torch.sum(MASK_TIME) > 0:
                    ...
                    loss_dist_o += torch.sum(torch.abs(calc_dist) * mask_pen / (num_pen + 1e-8)) * 10  # num_pen+1e-8)

                OP_MASK_TIME = ~MASK_TIME
                # num_verts_small=HAND_SMALL_INDEXES[i].shape[0]
                sbj2obj_mask_first = sbj2obj * WHETHER_TOUCH
                contact_ids = HAND_SMALL_INDEXES[i][HAND_SMALL_INDEXES[i] < 778]

                sd_closer = sbj2obj_mask_first[OP_MASK_TIME][:, contact_ids]  # /num_verts_small
            ...
            if torch.sum(MASK_TIME) > 0:
                ...
                mask_pen = ((sd_pen - thresh) < 0).float() * (torch.abs(sd_pen) < 0.03).float()
                num_pen = torch.sum(mask_pen, dim=-1).reshape(-1, 1)
                loss_dist_o += torch.sum(torch.abs(calc_dist) * mask_pen / (num_pen + 1e-8)) * 50  # num_pen+1e-8)
            ...
            loss_pinky_thumb_out_notmid = restrict_angles(thumb_pinky_out_notmid, theta_max1_1, theta_min1_1, mode=1,
                                                          flag='1')
            ...
            loss_pinky_thumb_out_mid = restrict_angles(thumb_pinky_out_mid, theta_max1_2, theta_min1_2, mode=1,
                                                       flag='2')
            ...
            theta_min2 = torch.tensor([[-0.8, -0.5, -0.8], [-0.4, -0.5, -0.8], [-0.5, -0.5, -0.8]]).reshape(1, 3,
                                                                                                            3).float().to(
                device)

            loss_pinky = restrict_angles(pinky_in, theta_max2, theta_min2, mode=1, flag='3')
            #                                                  -0.1
            theta_max3 = torch.tensor([[0.45, 0.45, 1.5], [0.45, 0.45, -0.1], [0.45, 0.45, 1.5]]).reshape(1, 3,
                                                                                                          3).float().to(
                device)

            theta_min3 = torch.tensor([[-0.5, -0.5, -0.2], [-0.5, -0.5, -0.8], [-0.5, -0.5, -0.8]]).reshape(1, 3,
                                                                                                            3).float().to(
                device)
            ...
            # ## restrict angles
            if left_or_right:
                d1 = 0.2 * torch.sum(((jtr[:, [41, 42, 44, 45, 50, 51, 41, 42]] - jtr[:, [44, 45, 50, 51, 47, 48, 53,
                                                                                          54]]) ** 2) * WHETHER_TOUCH_L.view(
                    -1, 1, 1))
                d2 = 0.2 * torch.sum(
                    ((verts[:, [7669, 7794, 7905]] - verts[:, [7794, 7905, 8022]]) ** 2) * WHETHER_TOUCH_L.view(-1, 1,
                                                                                                                1))
            else:
                d1 = 0.2 * torch.sum(((jtr[:, [26, 27, 29, 30, 35, 36, 26, 27]] - jtr[:, [29, 30, 35, 36, 32, 33, 38,
                                                                                          39]]) ** 2) * WHETHER_TOUCH_L.view(
                    -1, 1, 1))
                d2 = 0.2 * torch.sum(
                    ((verts[:, [4933, 5058, 5169]] - verts[:, [5058, 5169, 5286]]) ** 2) * WHETHER_TOUCH_L.view(-1, 1,
                                                                                                                1))

            loss_rot_reg += 2 - d1 - d2

            # smoothing
            loss_wrist_reg = torch.sum((wrist_rec ** 2)) * 10

            if epoch > 100:
                hand_verts = verts[:, hand_idx] - jtr[:, 0:1, :]

                loss_hand_pose_v_reg = smooth_mask_pose(hand_pose_rec, (WHETHER_TOUCH).view(-1, 1, 1)) * 4
                loss_hand_pose_v_reg += smooth_mask_pose(hand_pose_rec,
                                                    (WHETHER_OPTIMIZE - WHETHER_TOUCH).view(-1, 1, 1)) * 4
                loss_hand_pose_v_reg += smooth_mask_pose(hand_pose_rec, 1 - WHETHER_OPTIMIZE.view(-1, 1, 1)) * 4

                ## Initialization reg
                loss_hand_pose_v_reg += 0.05 * torch.sum(
                    (hand_pose_rec - hand_mean_single.view(1, -1, 3)) ** 2 * (1 - WHETHER_TOUCH_L).view(-1, 1, 1))
            ...
            if epoch > 200:
                CONTACT_MASK = (torch.abs(sbj2obj) < 0.01).unsqueeze(
                    2).float().detach()  # *((sbj2obj)>=0).unsqueeze(2).float().detach()
                delta_inv_minus = torch.einsum('tij,tjk->tik', (
                            verts[:, hand_idx] - sbj2obj_vector.detach() - obj_trans_tensor.reshape(-1, 1, 3)),
                                               obj_rot_inv_tensor.permute(0, 2, 1))  # T,3,3
                ## Contact Sliding Loss
                if epoch % 2 == 0:
                    delta_temporal_inv_minus = ((delta_inv_minus[1:] - delta_inv_minus[:-1]) * CONTACT_MASK[
                                                                                               1:] * WHETHER_TOUCH[
                                                                                                     1:].view(-1, 1,
                                                                                                              1)) ** 2  ##T,N,3
                else:
                ...
                if left_or_right:
                    hand_joints = jtr[:, 40:55, :]
                    centroid = (jtr[:, 21, :] + jtr[:, 40, :] + jtr[:, 46, :]) / 3
                else:
                    hand_joints = jtr[:, 25:40, :]
                    centroid = (jtr[:, 20, :] + jtr[:, 45, :] + jtr[:, 31, :]) / 3

                transformed_joints = torch.matmul(hand_joints - obj_trans_tensor[:, None, :], obj_rot_mat_tensor)
                transformed_wrists = torch.matmul(centroid[:, None, :] - obj_trans_tensor[:, None, :], obj_rot_mat_tensor)

                optim_mask = (WHETHER_OPTIMIZE.view(-1) > 0.5).float()
                touch_mask = (WHETHER_TOUCH.view(-1) > 0.5).float()
                pair_mask = (optim_mask[:-1] * optim_mask[1:]).view(-1, 1, 1)

                if pair_mask.numel() > 0:
                    wrist_delta = transformed_wrists[1:] - transformed_wrists[:-1]
                    wrist_smooth_loss = torch.sum((wrist_delta ** 2) * pair_mask) * 100
                    loss_hand_pose_v_reg += wrist_smooth_loss

                pair_mask = (touch_mask[:-1] * touch_mask[1:]).view(-1, 1, 1)

                if pair_mask.numel() > 0:
                    joints_delta = transformed_joints[1:] - transformed_joints[:-1]

                    wrist_smooth_loss = torch.sum((joints_delta ** 2) * pair_mask) * 500
                    loss_hand_pose_v_reg += wrist_smooth_loss

                if left_or_right:
                    loss_hand_pose_v_reg += smooth_mask_pose(
                        torch.cat([body_n_pose[:, 18 * 3:19 * 3], body_n_pose[:, 20 * 3:21 * 3]], dim=-1),
                        (WHETHER_TOUCH).view(-1, 1)) * 1
                    loss_hand_pose_v_reg += smooth_mask_pose(
                        torch.cat([body_n_pose[:, 18 * 3:19 * 3], body_n_pose[:, 20 * 3:21 * 3]], dim=-1),
                        (WHETHER_OPTIMIZE.view(-1, 1) - WHETHER_TOUCH).view(-1, 1)) * 10
                    loss_hand_pose_v_reg += smooth_mask_pose(
                        torch.cat([body_n_pose[:, 18 * 3:19 * 3], body_n_pose[:, 20 * 3:21 * 3]], dim=-1),
                        1 - WHETHER_OPTIMIZE.view(-1, 1)) * 10
                else:
                    loss_hand_pose_v_reg += smooth_mask_pose(
                        torch.cat([body_n_pose[:, 17 * 3:18 * 3], body_n_pose[:, 19 * 3:20 * 3]], dim=-1),
                        (WHETHER_TOUCH).view(-1, 1)) * 1
                    loss_hand_pose_v_reg += smooth_mask_pose(
                        torch.cat([body_n_pose[:, 17 * 3:18 * 3], body_n_pose[:, 19 * 3:20 * 3]], dim=-1),
                        (WHETHER_OPTIMIZE.view(-1, 1) - WHETHER_TOUCH).view(-1, 1)) * 10
            ...
            loss_v_reg = loss_hand_pose_v_reg + loss_rot_reg + loss_wrist_reg
            ...
            loss_dict['contact'] = loss_touch.detach().cpu().numpy()
            loss_dict['reg'] = loss_v_reg.detach().cpu().numpy()
            loss_dict['smooth'] = loss_hand_pose_v_reg.detach().cpu().numpy()
        return loss, loss_dict

    def calc_loss_arm(verts, jtr, epoch, left_or_right, body_n_pose):
        if left_or_right:  ## LEFT HAND OR RIGHT HAND
            WHETHER_TOUCH = WHETHER_TOUCH_RIGHT
            WHETHER_TOUCH_L = WHETHER_TOUCH_RIGHT_L
            frame_times_touch = frame_times_right
        ...
        with torch.enable_grad():
            ...
            for i in range(15, 16):
                ...
                if torch.sum(MASK_TIME) > 0:
                    ...
                    mask_pen = ((sd_pen - thresh) < 0).float() * (torch.abs(sd_pen) < 0.03).float()
                    num_pen = torch.sum(mask_pen, dim=-1).reshape(-1, 1)
                    loss_dist_o += torch.sum(torch.abs(calc_dist) * mask_pen / (num_pen + 1e-8)) * 5  # num_pen+1e-8)


            sbj2obj_mask_first = sbj2obj * WHETHER_TOUCH

            sd_closer = sbj2obj_mask_first[:, palm_center]  # /num_verts_small
            far_mask = sd_closer > 0.05

            loss_touch += 5 * torch.sum(torch.abs(sd_closer * far_mask) ** 2)
            ...
            transformed_wrists = torch.matmul(wrists - obj_trans_tensor[:, None, :], obj_rot_mat_tensor)
            ...
            if pair_mask.numel() > 0:
                wrist_delta = transformed_wrists[1:] - transformed_wrists[:-1]
                wrist_smooth_loss = torch.sum((wrist_delta ** 2) * pair_mask) * 500
                loss_hand_pose_v_reg += wrist_smooth_loss

            loss_hand_pose_v_reg += smooth_mask_pose(body_n_pose[:, 17 * 3:21 * 3], (WHETHER_TOUCH).view(-1, 1, 1)) * 10
            loss_hand_pose_v_reg += smooth_mask_pose(body_n_pose[:, 17 * 3:21 * 3],
                                                     (WHETHER_OPTIMIZE - WHETHER_TOUCH).view(-1, 1, 1)) * 10
            loss_hand_pose_v_reg += smooth_mask_pose(body_n_pose[:, 17 * 3:21 * 3], 1 - WHETHER_OPTIMIZE.view(-1, 1, 1)) * 10

            loss_wrist_reg = torch.sum((wrist_rec ** 2)) * 20
            loss_hand_pose_v_reg += loss_wrist_reg

            loss = loss_dist_o + loss_touch + loss_hand_pose_v_reg
    ...
    def calc_loss_w_wrist(hand_pose_rec, wrist_rec, epoch, optim_hand=True):
        body_n_pose = body_pose.clone()
        body_n_pose[:, 17 * 3:21 * 3] += wrist_rec
        SBJ_OUTPUT = sbj_m(pose_body=body_n_pose,
        ...
        if optim_hand:
            loss_left, loss_dict_left = calc_loss(verts, jtr, hand_pose_rec[:, :15, :], epoch, 0, body_n_pose)
            loss_right, loss_dict_right = calc_loss(verts, jtr, hand_pose_rec[:, 15:, :], epoch, 1, body_n_pose)
        else:
            loss_left, loss_dict_left = calc_loss_arm(verts, jtr, epoch, 0, body_n_pose)
            loss_right, loss_dict_right = calc_loss_arm(verts, jtr, epoch,1, body_n_pose)
        loss_dict_all = {}
        ...
        return loss_all * LOSS_RATIO, loss_dict_all, None

    def interpolate_pose_outside_touch_all(body_pose_w, touch_mask, left_or_right):
        """
        touch mask 内保持不变；
        touch mask 外所有帧，用最近的左右 contact pose 做插值

        Args:
            body_pose_w: (T, 63) torch tensor
            touch_mask: (T, 1) or (T,)
            left_or_right: 0 for left, 1 for right
        Returns:
            body_pose_out
        """
        body_pose_out = body_pose_w.clone()
        touch = (touch_mask.view(-1) > 0.5).detach().cpu().numpy().astype(np.int32)
        T = touch.shape[0]

        if left_or_right:
            joint_slices = [
                slice(18 * 3, 19 * 3),  # right elbow
                slice(20 * 3, 21 * 3),  # right wrist
            ]
        else:
            joint_slices = [
                slice(17 * 3, 18 * 3),  # left elbow
                slice(19 * 3, 20 * 3),  # left wrist
            ]

        touch_ids = np.where(touch == 1)[0]
        if len(touch_ids) == 0:
            return body_pose_out

        prev_touch = np.full(T, -1, dtype=np.int32)
        next_touch = np.full(T, -1, dtype=np.int32)
        ...
        for t in range(T):
            ...
            for joint_slice in joint_slices:
                if l != -1 and r != -1 and l != r:
                    ...
                    left_pose = body_pose_out[l, joint_slice].clone()
                    right_pose = body_pose_out[r, joint_slice].clone()
                    body_pose_out[t, joint_slice] = (1.0 - alpha) * left_pose + alpha * right_pose
                elif l != -1:
                    body_pose_out[t, joint_slice] = body_pose_out[l, joint_slice].clone()
                elif r != -1:
                    body_pose_out[t, joint_slice] = body_pose_out[r, joint_slice].clone()
    ...
    wrist_rec = Variable(torch.zeros_like(body_pose[:, 17 * 3:21 * 3]), requires_grad=True)

    # optimizer_arm = optim.Adam([{'params': [wrist_rec], 'lr': 0.001}])
    #
    # tqdm_bar = tqdm(range(500))
    # for ii in tqdm_bar:
    #     optimizer_arm.zero_grad()
    #     loss, loss_dict, endflag = calc_loss_w_wrist(hand_pose_rec, wrist_rec, ii, optim_hand=False)
    #     if endflag:
    #         break
    #     # losses_str = ' '.join(['{}: {:.4f} | '.format(x, loss_dict[x]) for x in loss_dict.keys()])
    #     tqdm_bar.set_description(
    #         f"Loss:{loss.item():.3f}, Smooth:{loss_dict['smooth'].item():.3f}, contact:{loss_dict['contact'].item():.3f} pene:{loss_dict['collision'].item():.3f}")
    #     loss.backward(retain_graph=False)
    #
    #     optimizer_arm.step()
    #     tmp_smplhparams['wrist_pose'] = copy.deepcopy(wrist_rec.detach())

    optimizer_hand = optim.Adam([{'params': [hand_pose_rec], 'lr': 0.001},
                                 {'params': [wrist_rec], 'lr': 0.001}])

    # tqdm_bar = tqdm(range(1000))
    for ii in range(1000):
        optimizer_hand.zero_grad()
        loss, loss_dict, endflag = calc_loss_w_wrist(hand_pose_rec, wrist_rec, ii, optim_hand=True)
        if endflag:
            break
        # losses_str = ' '.join(['{}: {:.4f} | '.format(x, loss_dict[x]) for x in loss_dict.keys()])
        # tqdm_bar.set_description(f"Loss:{loss.item():.3f}, Smooth:{loss_dict['smooth'].item():.3f}, reg:{loss_dict['reg'].item():.3f} pene:{loss_dict['collision'].item():.3f} contact:{loss_dict['contact'].item():.3f}")
        loss.backward(retain_graph=False)
    ...
    body_pose_w = body_pose.clone()
    body_pose_w[:, 17 * 3:21 * 3] += tmp_smplhparams['wrist_pose']
    # body_pose_w = interpolate_pose_outside_touch_all(body_pose_w, WHETHER_TOUCH_LEFT, left_or_right=0)
    # body_pose_w = interpolate_pose_outside_touch_all(body_pose_w, WHETHER_TOUCH_RIGHT, left_or_right=1)
    body_pose_w = body_pose_w.detach().cpu().numpy()

    os.makedirs(export_file, exist_ok=True)
    with np.load(human_npz_path, allow_pickle=True) as f:
        poses, betas, trans, gender = f['poses'], f['betas'], f['trans'], str(f['gender'])
    poses[:, -30 * 3:] = hand_pose
    poses[:, 3:66] = body_pose_w
    save_path = os.path.join(export_file, 'human.npz')
    np.savez(save_path, **{'poses': poses, 'betas': betas, 'trans': trans, 'gender': np.array(gender)})

    save_path = os.path.join(export_file, 'loss.pkl')
...
if __name__ == '__main__':
    ...
    root_path = 'updated_fix'
    res_path = "updated_results"

    motion_ls = sorted(os.listdir(root_path))
    motion_ls = [ptr for ptr in motion_ls if not os.path.exists(os.path.join(res_path, ptr, 'human.npz'))]
    data_len = len(motion_ls)

    print("Sequence Number:", len(motion_ls))
    for fn in tqdm(sorted(motion_ls)):
        name = os.path.join(root_path, fn)
        optimize1(name, fn, res_path)
