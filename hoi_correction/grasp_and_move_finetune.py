...
def optimize1(name, fname=''):
    ...
    SMPLX_PATH = './models/smplx'
    export_file = f"updated_results/{fname}"
        ...
        with torch.enable_grad():
            ...
            for i in range(16):
                ...
                if torch.sum(MASK_TIME) > 0:
                    ...
                    loss_dist_o += torch.sum(torch.abs(calc_dist) * mask_pen / (num_pen + 1e-8)) * 15  # num_pen+1e-8)
            ...
            if epoch > 200:
                ...
                loss_hand_pose_v_reg += smooth_mask_pose(body_n_pose[:, 17 * 3:21 * 3],
                                                         (WHETHER_TOUCH).view(-1, 1, 1)) * 4
                loss_hand_pose_v_reg += smooth_mask_pose(body_n_pose[:, 17 * 3:21 * 3],
                                                         (WHETHER_OPTIMIZE - WHETHER_TOUCH).view(-1, 1, 1)) * 10
                loss_hand_pose_v_reg += smooth_mask_pose(body_n_pose[:, 17 * 3:21 * 3],
                                                         1 - WHETHER_OPTIMIZE.view(-1, 1, 1)) * 10
    ...
    def calc_loss_arm(verts, jtr, epoch, left_or_right, body_n_pose):
        ...
        with torch.enable_grad():
            ...
            if pair_mask.numel() > 0:
                wrist_delta = transformed_wrists[1:] - transformed_wrists[:-1]
                wrist_smooth_loss = torch.sum((wrist_delta ** 2) * pair_mask) * 15
                loss_hand_pose_v_reg += wrist_smooth_loss

            loss_hand_pose_v_reg += smooth_mask_pose(body_n_pose[:, 17 * 3:21 * 3], (WHETHER_TOUCH).view(-1, 1, 1)) * 4
            loss_hand_pose_v_reg += smooth_mask_pose(body_n_pose[:, 17 * 3:21 * 3],
                                                     (WHETHER_OPTIMIZE - WHETHER_TOUCH).view(-1, 1, 1)) * 4
            loss_hand_pose_v_reg += smooth_mask_pose(body_n_pose[:, 17 * 3:21 * 3], 1 - WHETHER_OPTIMIZE.view(-1, 1, 1)) * 4
...
if __name__ == '__main__':
    ...
    root_path = '/move/u/dixuan/pj/InterAct/updated_fix'
    res_path = "/move/u/dixuan/pj/InterAct/updated_results"

    motion_ls = ["sub1_clothesstand_068", "sub9_floorlamp_012", "sub9_monitor_055", "sub7_woodchair_026", "sub12_vacuum_000", "sub11_monitor_003"]
    # motion_ls = ["sub12_vacuum_000", "sub9_floorlamp_012"]
