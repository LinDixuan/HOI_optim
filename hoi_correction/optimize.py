...
def optimize1(name,fname=''):
    ...
    # touch_indices[os.path.basename(name)] = {"right": WHETHER_TOUCH_RIGHT.int().detach().cpu().numpy().tolist(),
    #                                          "left": WHETHER_TOUCH_LEFT.int().detach().cpu().numpy().tolist()}
    ...
    def calc_loss(verts,jtr,hand_pose_rec,epoch,left_or_right):
        ...
        with torch.enable_grad():       
            ...
            # hand_v = verts[50,hand_idx]
            # human_v = verts[50]
            # print(hand_v.shape)
            # hand_balls = make_balls(hand_v.detach().cpu().numpy(),0.008, colors = [255, 0, 0])
            # hand_balls = trimesh.points.PointCloud(vertices=hand_v.detach().cpu().numpy())
            # human_mesh = trimesh.Trimesh(vertices=human_v.detach().cpu().numpy(), faces=sbj_face, process=False)
            # hand_balls.export("/home/dixuan/sony/InterAct/test_res/hand.ply")
            # human_mesh.export("/home/dixuan/sony/InterAct/test_res/man.ply")
            ...
            for i in range(16):
                ...
                # part_i = hand_v[HAND_SMALL_INDEXES[i][HAND_SMALL_INDEXES[i] < 778]]
                # part_i_balls = trimesh.points.PointCloud(vertices=part_i.detach().cpu().numpy())
                # part_i_balls.export(f"/home/dixuan/sony/InterAct/test_res/hand_small_{i:02d}.ply")
                ...
                sd_closer=sbj2obj_mask_first[OP_MASK_TIME][:,HAND_SMALL_INDEXES[i][HAND_SMALL_INDEXES[i]<778]]#/num_verts_small
                loss_touch+=10*torch.sum(torch.abs(sd_closer)**2)
    ...
    export_file = f"omomo_hand_correct/sequences_canonical/{fname}"
...
if __name__ == '__main__':
    ...
    root_path = 'omomo/sequences_canonical'

    for i,fn in tqdm(enumerate(sorted(os.listdir(root_path)))):
        name=os.path.join(root_path,fn)
        optimize1(name,fn)

    # with open("./touch.json",  "w", encoding="utf-8") as f:
    #     json.dump(touch_indices, f)
